...
==================
Prepara il dataset LUDB in formato tensore PyTorch per la rete V2.
Mantiene il filtro Passa-Banda (Bandpass Filtering) 0.5Hz - 45Hz.
Cambia semplicemente la cartella di output predefinita.
"""
...
def setup_args():
    parser = argparse.ArgumentParser(description="Prepara dataset tensore LUDB (V2)")
    parser.add_argument("-d", "--dir", default="data", help="Directory WFDB")
    # Usa un nuovo target output directory per le feature _2
    parser.add_argument("-o", "--output", default="ludb_tensors_2", help="Output directory")
...
def main():
    args = setup_args()
    print("=" * 60)
    print("  LUDB -> Tensor Dataset Miorato [V2 - Bandpass]")
    print("=" * 60)
