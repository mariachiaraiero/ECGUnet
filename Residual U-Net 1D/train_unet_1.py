...
train_unet_1.py
=============
Script di addestramento migliorato (ResU-Net 1D).
Modifiche rispetto all'originale:
- Usa ecg_unet_1.py (ResUNet).
- Data Augmentation Avanzata (Random Noise, Scaling).
- Loss function combinata: CrossEntropy + Dice Loss (fondamentale per lo squilibrio delle onde).
"""
...
from ecg_unet_1 import ECGUNet  # Usa il nuovo modello

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset", type=str, default="ludb_tensors_1/ludb_dataset.pt", help="Dataset file")
    ...
    parser.add_argument("--output-dir", type=str, default="checkpoints_1")
    return parser.parse_args()


# ============================================================
#  Combined Loss: CELoss + DiceLoss
# ============================================================
class CEDiceLoss(nn.Module):
    """
    Combinazione di CrossEntropy e Multi-class Dice Loss.
    La Dice aiuta a superare l'estremo sbilanciamento delle onde ECG (P e T sono corte).
    """
    def __init__(self, ce_weight, num_classes=4):
        super().__init__()
    ...
    def forward(self, logits, targets):
        ...
        dice_loss = 0.0
        # Evitiamo di inserire il background nel Dice se vogliamo dare focus solo alle onde,
        # ma di norma si fa la media su tutto o solo sulle classi foreground (1, 2, 3)
        ...
        dice_loss = dice_loss / (self.num_classes - 1) # media sulle classi foreground
...
def apply_data_augmentation(X_batch):
    """Aggiunge augmentation casuali ai batch in addestramento."""
    B, C, L = X_batch.shape
    device = X_batch.device
    
    # Random Scaling (Amplitude) +/- 15%
    ...
    # Vert Shift (Baseline wander locale) +/- 0.1
...
def train_one_epoch(model, loader, criterion, optimizer, device):
    ...
    for X, Y in loader:
        ...
        X = X.to(device)
        Y = Y.to(device)
        X = apply_data_augmentation(X) # <--- Nuova Augmentation
...
def main():
    ...
    print(f"Training su {device} - Dataset {X.shape}")
    
    model = ECGUNet(in_channels=1, num_classes=4, base_filters=8).to(device)
    
    # Nuova loss combinata CE + Dice
    criterion = CEDiceLoss(ce_weight=data['class_weights'].to(device), num_classes=4)
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=2)

    os.makedirs(args.output_dir, exist_ok=True)
    best_path = os.path.join(args.output_dir, "best_ecg_unet_1.pt")
