...
class ECGUNet(nn.Module):
    ...
    def __init__(self, in_channels=1, num_classes=4, base_filters=64):
        super().__init__()

        f = base_filters  # 64

        # ===== Encoder =====
        self.enc1 = ConvBlock(in_channels, f)          # 1  -> 64
        self.enc2 = ConvBlock(f,     f * 2)             # 64 -> 128
        self.enc3 = ConvBlock(f * 2, f * 4)             # 128 -> 256
        self.enc4 = ConvBlock(f * 4, f * 8)             # 256 -> 512
        ...
        self.upconv4 = nn.ConvTranspose1d(f * 16, f * 8, kernel_size=8, stride=2, padding=3)
        self.dec4    = ConvBlock(f * 16, f * 8)         # 1024 (concat) -> 512

        self.upconv3 = nn.ConvTranspose1d(f * 8, f * 4, kernel_size=8, stride=2, padding=3)
        self.dec3    = ConvBlock(f * 8, f * 4)          # 512 (concat) -> 256

        self.upconv2 = nn.ConvTranspose1d(f * 4, f * 2, kernel_size=8, stride=2, padding=3)
        self.dec2    = ConvBlock(f * 4, f * 2)          # 256 (concat) -> 128

        self.upconv1 = nn.ConvTranspose1d(f * 2, f, kernel_size=8, stride=2, padding=3)
        self.dec1    = ConvBlock(f * 2, f)              # 128 (concat) -> 64
    ...
    def forward(self, x):
        ...
        e2 = self.enc2(self.pool(e1))   # (B, 128,  L/2)
        e3 = self.enc3(self.pool(e2))   # (B, 256,  L/4)
        e4 = self.enc4(self.pool(e3))   # (B, 512,  L/8)

        # ===== Bottleneck =====
        b = self.bottleneck(self.pool(e4))  # (B, 1024, L/16)

        # ===== Decoder con skip connections =====
        # Livello 4
        d4 = self.upconv4(b)                        # (B, 512, ~L/8)
        d4 = self._pad_to_match(d4, e4.size(2))     # zero-pad per allineare a e4
        d4 = torch.cat([d4, e4], dim=1)             # (B, 1024, L/8)
        d4 = self.dec4(d4)                          # (B, 512,  L/8)

        # Livello 3
        d3 = self.upconv3(d4)                       # (B, 256, ~L/4)
        d3 = self._pad_to_match(d3, e3.size(2))
        d3 = torch.cat([d3, e3], dim=1)             # (B, 512, L/4)
        d3 = self.dec3(d3)                          # (B, 256, L/4)

        # Livello 2
        d2 = self.upconv2(d3)                       # (B, 128, ~L/2)
        d2 = self._pad_to_match(d2, e2.size(2))
        d2 = torch.cat([d2, e2], dim=1)             # (B, 256, L/2)
        d2 = self.dec2(d2)                          # (B, 128, L/2)

        # Livello 1
        d1 = self.upconv1(d2)                       # (B, 64, ~L)
        d1 = self._pad_to_match(d1, e1.size(2))
        d1 = torch.cat([d1, e1], dim=1)             # (B, 128, L)
